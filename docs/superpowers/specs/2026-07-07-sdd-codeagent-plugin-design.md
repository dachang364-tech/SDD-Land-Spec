...
- 把文档（`spec.md`、`feature-*.md`、`bugfix-*.md`、ADR、PRD）作为一等资产，存放在 `docs/vX.Y.Z/` 之下。
...
Plugin 内部所有面向用户的字符串 —— Skill 正文、命令输出、状态报告、模板（`spec.md`、`feature-*.md`、`bugfix-*.md`、ADR、PRD）、错误消息、commit-message 建议 —— 一律使用**简体中文**。以下内容保留英文：
...
│   /sdd.adr /sdd.status /sdd.archive /sdd.doctor             │
├────────────────────────────────────────────────────────────┤
│  Skills（承载实际方法论）                                    │
...
│   sdd-adr-writer / sdd-status-reader / sdd-archiver         │
│   sdd-doctor-runner                                         │
├────────────────────────────────────────────────────────────┤
│  Hooks（路径 / 状态守卫）                                    │
...
│   .* revision meta (last_change_type, last_changed_at,          │
│     needs_rework, rework_since)                                 │
└────────────────────────────────────────────────────────────┘
           ↓ 调用 ↓
┌────────────────────────────────────────────────────────────┐
│  外部框架（不重新实现）                                       │
...
NONE --/sdd.init--> INITED --/sdd.new vX.Y.Z--> RESEARCH --/sdd.prd--> PRD --/sdd.spec--> SPEC
                                                                                          |
                                                                                          | /sdd.plan <name>     (per-feature, repeatable; first /sdd.plan bugfix-<ID> for bugfix)
                                                                                          v
...
flowchart TD
    ...
    SPEC -- "/sdd.plan &lt;name&gt;<br/>(per-feature / bugfix, repeatable)" --> FEATURE_PLAN["FEATURE_PLAN"]
...
```json
{
  ...
  "artifacts": {
    "research": {
      ...
      "last_change_type": "patch",
      "last_changed_at": "2026-07-07T08:00:00Z",
      "needs_rework": [],
      "rework_since": null
    },
    "prd": {
      ...
      "last_change_type": "breaking",
      "last_changed_at": "2026-07-07T09:30:00Z",
      "needs_rework": ["feature-login", "feature-payment"],
      "rework_since": "2026-07-07T09:30:00Z"
    },
    "spec": {
      ...
      "last_change_type": "patch",
      "last_changed_at": "2026-07-07T10:00:00Z",
      "needs_rework": [],
      "rework_since": null
    },
    "features": [
      ...
      { "name": "bugfix-0002-fix-null", "path": "docs/v1.0.1/plans/bugfix-0002-fix-null.md", "status": "draft" }
    ],
    "adrs": [
...
**status 取值**：`missing` | `draft` | `approved` | `deprecated`。`/sdd.new` 不预创建 `prd.md`，因此新建版本后 `artifacts.prd.status = "missing"`；`/sdd.prd` 完成后变 `draft`，用户批准后变 `approved`。`spec` / `features.*` 同理。`features` 数组同时承载 feature plan 与 bugfix plan：`{ name: "feature-login", status: "planned" }` 与 `{ name: "bugfix-0001-fix-null", status: "coding" }` 共存。

**修订元数据**（v0.1 新增，见 §3.2）：
- `last_change_type`：`"patch"`（契约内补丁，下游不重做）| `"breaking"`（契约外变更，下游标 dirty）。初始为 `null`，首次写产物时不设。
- `last_changed_at`：ISO 8601 时间戳。初始为 `null`。
...
/sdd.<目标阶段>    :  /sdd.spec  /sdd.plan <name>  /sdd.prd ...   （首次写与变更同入口）
/sdd.plan <mode>  :  /sdd.plan bugfix-<ID>                       （显式生成 bugfix plan）
```

**变更流程四步**：

1. **发起**：Skill 读取目标产物（path 来自 `artifacts.<x>.path`）。若不存在 → 走「首次写」流程（与阶段门控点连用）；若存在 → 进入变更流程。
2. **分类（Skill 自动判断 + 用户确认）**：Skill 对产物做启发式 diff，并把建议分类呈现给用户：

   | 建议分类   | 判据                                                                                                  | 示例                                                           |
   | ---------- | ----------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
   | `patch`    | 不改变 spec 内 User Story 的验收标准、不改接口；仅 typo、补文案、扩文案                                  | 把「邮箱已注册」→「该邮箱已被注册」                           |
   | `breaking` | 改变验收标准、删除 / 新增 User Story、改接口、改 schema 字段                                            | 把「邮箱」改「手机号」；删掉「禁止重复注册」验收                |

   用户可确认或修正。

3. **执行**：

   - **`patch`**：直接覆盖产物；更新 `artifacts.<x>.updated_at`、`last_change_type = "patch"`、`last_changed_at = now`。
   - **`breaking`**：覆盖产物 + 标记 `artifacts.<x>.needs_rework = [受影响下游列表]`、`rework_since = now`。
     - `prd` 变更：受影响下游 = 所有尚未归档的 `spec` 与 `plan`
     - `spec` 变更：受影响下游 = 引用的所有 `feature-*.md`（按 spec 章节匹配）
     - `plan` 变更：受影响下游 = 该 plan 已派发但未完成的 task 列表（在 `.sdd/progress.md` 中检索）

4. **通知下游 + subagent 反应**：

   - **`patch`** 后：下游 subagent 不受影响；无需特殊处理。
   - **`breaking`** 后：subagent dispatch 时，`sdd-code-orchestrator` 读 `state.json.artifacts.spec.needs_rework`，若当前 feature 在列 → **主动告警**「本 plan 上游 spec 已修订（commit X、breaking），是否先 review？」，由主 Agent + 用户决定下一步，**不**自动重启也不拒绝派发。

**契约 vs 修订元数据的关系**：

| 维度              | 来源                                       | 持久化                                          |
| ----------------- | ------------------------------------------ | ----------------------------------------------- |
| 契约（spec 内容） | `docs/vX.Y.Z/specs/spec.md` 文件本身       | git 历史                                        |
| 修订元数据        | state.json（`last_change_type` 等四个字段） | state.json 文件                                  |
...
│   │   │   ├── feature-payment.md
│   │   │   └── bugfix-0002-fix-null.md # bugfix plan（由变更流程触发）
│   │   └── decisions/
│   │       ├── 0001-use-postgresql.md  # ADR
│   │       └── bugfix-0002-fix-null.md # bugfix 记录（变更流程识别为 bug 时自动创建）
│   └── archive/
...
spec.md                <- 功能层 + 项目层元信息：每个 feature 的 User Story（P1/P2/P3）与验收标准（Spec-Kit Given-When-Then）+ 融合 Spec-Kit plan-template 的项目层章节（Technical Context / Constitution Check / Project Structure）
  | 按 feature / bugfix 拆实现
feature-<name>.md      <- 单 feature 实现层：user story -> task，含 TDD 步骤 + commit 粒度（N 份）
bugfix-<ID>.md         <- bugfix 实现层：由变更流程触发，结构同 feature-<name>.md
  | 派给 subagent
源码 + 提交
```
...
| 维度           | `spec.md`                                                 | `feature-<name>.md` / `bugfix-<ID>.md`              |
| -------------- | --------------------------------------------------------- | --------------------------------------------------- |
| 数量           | 1 份（按版本）                                            | 按需 N 份                                           |
| 粒度           | 粗：User Story、验收标准、Technical Context / Constitution / Project Structure | 细：具体文件、具体任务、TDD 步骤、commit 粒度 |
| 回答的问题     | "vX.Y.Z 整体要做什么？整体技术语境与项目结构是什么？"      | "feature-X / bugfix-<ID> 具体怎么落地？"             |
...
| `/sdd.plan <name>` | `sdd-plan-writer`           | `docs/vX.Y.Z/plans/feature-<name>.md` 或 `bugfix-<ID>.md` | spec 已批准             |
...
| Feature/Bugfix Plan | `sdd-plan-writer`          | Superpowers `writing-plans` 流程（文件级任务、TDD 步骤、commit 粒度）；bugfix 模式走同一 Skill，复用 `writing-plans` |
...
- **bugfix 模式**（`/sdd.plan bugfix-<ID>`，由变更流程 / 自然语言触发）：
...
| 11 个 Skill 目录                    | `skills/sdd-{init-runner,new-version-bootstrapper,research-writer,prd-writer,spec-writer,plan-writer,code-orchestrator,adr-writer,archiver,status-reader,doctor-runner}/SKILL.md` 全部存在 |
| 11 个 command 文件                  | `commands/sdd.{init,new,research,prd,spec,plan,code,adr,status,archive,doctor}.md` 全部存在 |
| Hooks 配置                          | `hooks/hooks.json` 合法 JSON，四个事件都注册（SessionStart / PreToolUse / PostToolUse / PreCompact） |
| Templates 齐备                      | `templates/{research,prd,spec,feature-plan,adr,bugfix}.md.tmpl` 全部存在 |
...
| `PreToolUse Write/Edit` | 即将写入                  | **仅检查文档类路径**：目标路径命中 `docs/vX.Y.Z/**` 时，按路径查「该路径对应的合法阶段集合」是否包含 `state.json.phase`。<br>- `research.md` ↔ `RESEARCH`<br>- `prd.md` ↔ `PRD`<br>- `specs/spec.md` ↔ `SPEC`<br>- `plans/feature-*.md` ↔ `FEATURE_PLAN`、`CODE`<br>- `plans/bugfix-*.md` ↔ `FEATURE_PLAN`、`CODE`<br>- `decisions/**` ↔ 任何阶段（ADR/bugfix 是横切产物）<br>**phase 上下跳的处理**：用户想从 SPEC 阶段回跳改 `prd.md`，**不能**直接 Edit —— 必须调用 `/sdd.prd` 等命令入口。该命令入口在内部把 `phase` 切到目标阶段后，再调起对应的 Skill 写文档；Hook 始终读到的是**已稳定**的 phase，不会看到「写之前的瞬态切换」。<br>越阶段 → 退出码 2 + 提示「请运行 `/sdd.<对应阶段>`」。**v0.1 不拦截源码路径**。 |
...
├── templates/                       # research / prd / spec / feature-plan / adr / bugfix
...
4. `/sdd.status` 在每个阶段都准确反映当前状态（包括 research / prd 状态、`features[*].status`，以及 `needs_rework` 标记）。
5. `/sdd.archive` 能把 `docs/v0.0.1/` 通过 `git mv` 迁移到 `docs/archive/v0.0.1/`（包括 `research.md` / `prd.md`），原位置不再保留，并清空 `state.json` 中该版本相关字段。

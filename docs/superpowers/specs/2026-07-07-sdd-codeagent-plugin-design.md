...
│   /sdd.research /sdd.prd /sdd.code /sdd.bugfix              │
...
│   sdd-code-orchestrator / sdd-bugfix-triage                 │
...
NONE --/sdd.init--> INITED --/sdd.new vX.Y.Z--> RESEARCH --/sdd.prd--> PRD --/sdd.spec--> SPEC
                                                                                          |
                                                                                          | /sdd.plan <name>     (per-feature, repeatable)
                                                                                          v
...
flowchart TD
    ...
    SPEC -- "/sdd.plan &lt;name&gt;<br/>(per-feature, repeatable)" --> FEATURE_PLAN["FEATURE_PLAN"]
...
**为什么去掉 TRD 阶段**：v0.1 砍掉 version-level 技术设计层（`trd.md` / `/sdd.trd`）。`spec.md` 已融合 Spec-Kit `plan-template.md` 的项目层章节（Technical Context / Constitution Check / Project Structure，见 §7.4），逐个 feature 的 plan 由 `/sdd.plan <name>` 直接生成 `plans/feature-<name>.md` 或 `plans/bugfix-<ID>.md`，与 Superpowers `writing-plans` 机制 1:1 对齐。**版本级阶段从 8 个缩短为 7 个**（砍掉 TRD）。
...
│   │   │   ├── feature-payment.md
│   │   │   └── bugfix-0002-fix-null.md # bugfix plan（由 /sdd.bugfix 触发）
│   │   └── decisions/
...
bugfix-<ID>.md         <- bugfix 实现层：由 /sdd.bugfix 触发，结构同 feature-<name>.md
  | 派给 subagent
源码 + 提交
```
...
| `/sdd.bugfix`      | `sdd-bugfix-triage`         | `decisions/bugfix-*.md` + 必要时调用 `/sdd.plan` 生成 `plans/bugfix-<ID>.md` | phase = CODE            |
...
> 编号自 7.0 起按仓库 `skills/` 目录的字典序编排。`sdd-init-runner`（7.0）、`sdd-new-version-bootstrapper`（7.1）属于项目级启动 Skill；`sdd-research-writer`～`sdd-doctor-runner`（7.2～7.11）属于版本级阶段 Skill。**12 个 Skill 一一对应 §11 列出的 12 个目录**。
...
- **bugfix 模式**（`/sdd.plan bugfix-<ID>`，由 `/sdd.bugfix` 决策树调用）：
  1. 读取 spec.md 与对应 `decisions/bugfix-<ID>.md`；后者必须先由 `sdd-bugfix-triage` 写好。
  2. 调用 **Superpowers `writing-plans`** 流程（同 feature 模式），但任务围绕 bug 修复。
  3. 写入 `docs/vX.Y.Z/plans/bugfix-<ID>.md`。
  4. 用户批准后，`artifacts.features[name=bugfix-<ID>].status = planned`，`state.json.active_bugfix = "bugfix-<ID>"`。
...
Q1：本次修复是否改变 spec 定义的行为或验收标准？
    是 -> 规范 bug -> 提议写 ADR
    否 -> 代码 bug -> 写 bugfix 记录 + 调起 /sdd.plan
```

处置路径：

- **代码 bug**
  - 写 `docs/vX.Y.Z/decisions/bugfix-<ID>-<title>.md`（MADR 风格，但使用「现象 / 根因 / 修复 / 影响」布局）。
  - 调用 `/sdd.plan bugfix-<ID>`，由 `sdd-plan-writer`（§7.5 bugfix 模式）生成 `plans/bugfix-<ID>.md`。
  - 由 `/sdd.code bugfix-<ID>` 进入标准 subagent + TDD 流程。
  - 把 `state.json.active_bugfix` 设为 `"bugfix-<ID>"`，bug 修复并 review 通过后清空。
- **规范 bug**
  - 写 ADR `000N-<title>.md`。
  - 用显式的 `[CHANGED]` diff 块更新 `spec.md`。
  - 对受影响的 feature 继续走 `/sdd.code` 流程（重新派发受影响 task）。
...
| 12 个 Skill 目录                    | `skills/sdd-{init-runner,new-version-bootstrapper,research-writer,prd-writer,spec-writer,plan-writer,code-orchestrator,bugfix-triage,adr-writer,archiver,status-reader,doctor-runner}/SKILL.md` 全部存在 |
| 12 个 command 文件                  | `commands/sdd.{init,new,research,prd,spec,plan,code,bugfix,adr,status,archive,doctor}.md` 全部存在 |

...
`/sdd:triage` 是 DR Advanced 的前置分诊 skill，详细设计见 [2026-07-13-triage-skill-spec.md](./2026-07-13-triage-skill-spec.md)。
...
输出必须包含置信度、已读取依据、原因、推荐路径和可选路径。除非用户明确授权，否则 ClaudeCode 不应直接创建 DR、修改 spec、修改 plan 或改代码。

分诊路径由独立 Triage Skill spec 维护，本规格只保留 DR Advanced 与 `/sdd:triage` 的流程关系。
...
原 feature/chg/arch DR 表达原始变更决策，不应承载后续发现的独立问题。后续问题默认使用新的 DR 表达；如果原 DR 已关闭，不重新打开原 DR。

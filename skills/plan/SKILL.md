...
- create：成功写入后必须显式调用 `/sdd:review <doc-path>`；`plan` 的 review mode 为 `quality -> feasibility`。新建文档拿不到有效 review 结果时保持 `draft`；若被阻断、需要用户确认或项目模板资产缺失，也保持 `draft`，不得绕过 gate 推进流程。

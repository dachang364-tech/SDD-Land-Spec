...
在唯一 active version 下创建或修订增量实现计划，路径位于 `docs/versions/vX.Y.Z/plan/`。
...
1. 如果 `<work-item>` 匹配 `^(00[1-9]|0[1-9][0-9]|[1-9][0-9][0-9])-(fix|feat|chg|arch)-[a-z0-9]+(-[a-z0-9]+)*$`，使用 code-class DR 模式。
2. 如果 `<work-item>` 匹配 `^(00[1-9]|0[1-9][0-9]|[1-9][0-9][0-9])-(spec|doc|typo)-[a-z0-9]+(-[a-z0-9]+)*$`，直接拒绝：`文档类 DR 不生成实现计划，不执行 /sdd:code。`
3. 如果 `<work-item>` 看起来像 DR（以三位数字和连字符开头），但不是合法完整 DR ID，即不符合 `001..999-<fix|feat|chg|arch|spec|doc|typo>-<lowercase-kebab-slug>` 形式，则显式失败：`无效 DR ID。` 不得落回 spec 模式。
4. 其他情况使用 spec 模式。
...
- 要求 `状态：accepted`、`class: code`、`plan_required: yes`、`code_required: yes`。
...
- create：文档生成仍由当前 Skill 负责；成功写入后由 `PostToolUse Hook` 触发 `scripts/lib/sdd-review-runner.sh` 这个共享 review runner。`plan` 的 runner mode 为 `quality -> feasibility`；新建文档拿不到有效 review 结果时保持 `draft`；若被阻断、需要用户确认或项目模板资产缺失，也保持 `draft`，不得绕过 gate 推进流程。
...
- 共享 runner 对 `plan` 自动按顺序触发 `quality -> feasibility`；每个 mode 的机器结果均须先通过 schema 校验。
...
- `Implementation Tasks` 必须是可由 agentic worker 直接执行的 TDD 手册，不是概要待办。

...
2. 加载模板 helper，确保后续可调用 `sdd_list_template_packs`、`sdd_default_template_pack` 与 `sdd_copy_template_pack`。
...
5. 调用 `sdd_list_template_packs "$plugin_root"` 获取模板包列表。
6. 将模板包列表展示给用户并请求选择。
7. 如果用户未显式切换，则调用 `sdd_default_template_pack` 取得默认值 `backend`。
8. 校验所选值属于可用模板包；若不是则失败。
9. 若所选值不是 `backend`，直接失败并说明当前仅实现 `backend`。
10. 复制 `CONSTITUTION.default.md` 与模板资产时，只在缺失时写入；不得覆盖用户已有内容。
11. 只有在模板包选择完成后才调用 `sdd_copy_template_pack "$plugin_root" "$project_root" "$selected_pack"`。
12. 如果 `sdd_copy_template_pack` 返回非零，停止并报告失败，不输出初始化成功结果。
13. 全部成功后，将所选模板包中的 `research / prd / spec / plan / dr` 运行时模板与标准文件物化到 `${CLAUDE_PROJECT_DIR}/.sdd/templates/`。
14. 不创建任何版本目录或版本级 `state.json`。
15. 不创建 `prd/prd.md`、`research/*.md`、`spec/*.md`、`plan/*.md` 或 `dr/*.md` 等正文文件。
16. 不修改 `CLAUDE.md` 或 `AGENTS.md`。
17. 只提示用户安装依赖插件，不执行 `scripts/install-deps.sh`。
18. 提醒用户本插件依赖 `superpowers` 与 `spec-kit`，请按 README 安装说明手动安装；`scripts/install-deps.sh` 仅作为可选辅助脚本。

---
name: code
description: Execute an SDD implementation plan or eligible lightweight fix DR. Use for /sdd:code whenever the user wants the plan run, a fix implemented, or a DR-driven code change started.
---
...
4. If input matches a code-class DR id `^(00[1-9]|0[1-9][0-9]|[1-9][0-9][0-9])-(fix|feat|chg|arch)-[a-z0-9]+(-[a-z0-9]+)*$`, first check for a matching plan by exact DR ID suffix. If no plan matches, read `docs/versions/vX.Y.Z/dr/<dr-id>.md` and use lightweight fix DR mode only when DR `tag` is `fix` and `plan_required: no`.
5. If input is `NNN`, match `docs/versions/vX.Y.Z/plan/NNN-*.md` and use plan execution mode.
...
- plan / DR 目标路径必须位于 `docs/versions/vX.Y.Z/plan/` 与 `docs/versions/vX.Y.Z/dr/`，不能再使用旧的 `plans/`、`decisions/`。
Steps:
...
- 不接受旧路径 `docs/versions/vX.Y.Z/plans/`、`docs/versions/vX.Y.Z/decisions/`、`docs/versions/vX.Y.Z/prd.md`、`docs/versions/vX.Y.Z/specs/`。
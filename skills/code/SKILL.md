...
1. If input matches a document-class DR id `^[0-9]{3}-(spec|doc|typo)-[a-z0-9-]+$`, refuse: `文档类 DR 不执行 /sdd:code。`
2. If input is DR-like (starts with three digits and a hyphen) but is not a valid full DR ID, fail explicitly: `无效 DR ID；必须使用 001..999-<fix|feat|chg|arch|spec|doc|typo>-<lowercase-kebab-slug>。` Do not fall through to plan or feature-name lookup.
3. If input matches a code-class DR id `^[0-9]{3}-(fix|feat|chg|arch)-[a-z0-9-]+$`, first check for a matching plan by exact DR ID suffix. If no plan matches, read `docs/versions/vX.Y.Z/decisions/<dr-id>.md` and use lightweight fix DR mode only when DR `tag` is `fix` and `plan_required: no`.
4. If input is `NNN`, match `docs/versions/vX.Y.Z/plans/NNN-*.md` and use plan execution mode.
5. If input is a complete plan basename, match the same `.md` basename and use plan execution mode.

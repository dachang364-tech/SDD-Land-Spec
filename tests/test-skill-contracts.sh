...
assert_contains "skills/review/SKILL.md" 'description: 作为受管 SDD 文档的唯一 review 编排入口'
...
assert_contains "skills/review/SKILL.md" '任一结果 `blocked: true`、`requires_user_confirmation: true` 或 `passed: false` 时，停止后续 mode'
assert_contains "skills/review/SKILL.md" '只有前一 mode `passed: true`、`blocked: false` 且不需要确认时才继续下一个 mode'
...
assert_contains "skills/spec/SKILL.md" '若被阻断、需要用户确认或项目模板资产缺失，也保持 `draft`'
assert_contains "skills/plan/SKILL.md" '若被阻断、需要用户确认或项目模板资产缺失，也保持 `draft`'
